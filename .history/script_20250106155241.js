...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('.room-list').addEventListener('click', function(event) {
        if (event.target.classList.contains('room-card')) {
            const selectedRoom = event.target.dataset.roomId;
            const roomName = event.target.dataset.roomName;
            const roomPrice = event.target.dataset.roomPrice;

            console.log("Selected Room:", roomName);  // Debug log to check room selection
    ...
    document.getElementById('bookingForm').addEventListener('submit', function(event) {
        ...
        bookingData.append('room_id', selectedRoom);
