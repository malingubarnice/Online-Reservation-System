...
document.addEventListener("DOMContentLoaded", function () {
    ...
    // Fetch room details from the database and populate the room type dropdown
    fetch('get_room.php')
        .then(response => response.json())
        .then(data => {
            data.forEach(room => {
                ...
                option.textContent = `${room.room_name} - KES ${room.price}`;
                roomSelect.appendChild(option);
            });
        })
        .catch(error => console.error('Error loading rooms:', error));

    // Update total price when check-in, check-out or room selection changes
    ...
    function updatePrice() {
        ...
        if (roomId && checkInDate.value && checkOutDate.value) {
            const room = getRoomById(roomId); // Function to get room data based on ID
            const duration = Math.ceil((checkOut - checkIn) / (1000 * 3600 * 24)); // Number of days

            if (duration > 0) {
                const totalPrice = room.price * duration;
                totalPriceInput.value = totalPrice.toLocaleString(); // Format the total price
            } else {
                totalPriceInput.value = ''; // Reset if the dates are invalid
            }
        }
    }

    // Helper function to find the selected room's details from the fetched data
