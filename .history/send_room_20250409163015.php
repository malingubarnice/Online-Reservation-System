...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $sql = "INSERT INTO bookings2 (booking_id, room_id, room_name, check_in_date, check_out_date, guest_count, contact_info, created_at) 
            VALUES (?, ?, ?, ?, ?, ?, ?, NOW())";
    $stmt = $conn->prepare($sql);
    ...
    if ($stmt->execute()) {
        ...
        try {
            ...
            // Recipient email
            $mail->addAddress($contactInfo); // User's contact email
