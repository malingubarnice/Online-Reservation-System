...
function selectRoom(roomId, roomName, price) {
    ...
    // Display selected room in the form
    document.getElementById('selectedRoom').value = roomName;
    calculateTotalPrice();  // Recalculate total price if necessary
}
...
function calculateTotalPrice() {
    ...
    if (checkInDate && checkOutDate) {
        ...
        if (checkOut >= checkIn) {
            ...
            const daysStayed = timeDiff / (1000 * 3600 * 24);  // Calculate days between dates

            const totalPrice = daysStayed * selectedRoomPrice;
...
document.getElementById('confirmBooking').addEventListener('click', function (event) {
    ...
    if (!checkInDate || !checkOutDate || !guestCount || !contactInfo || !totalPrice || totalPrice === "Invalid Dates") {
    ...
    // Use AJAX to submit the data to book_room.php
    fetch('book_room.php', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        alert(data);  // Shows either success or error message
    })
    .catch(error => {
        alert('Error: ' + error);
    });
