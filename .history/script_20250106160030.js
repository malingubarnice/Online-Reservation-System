...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.querySelector('.room-list').addEventListener('click', function(event) {
        if (event.target.classList.contains('room-card')) {
            ...
            // Update the form fields with the selected room data
            document.getElementById('selectedRoom').value = roomName;
            document.getElementById('totalPrice').value = roomPrice;
            document.getElementById('selectedRoom').dataset.roomId = selectedRoomId;  // Store room id for submission
        }
    });
    ...
    document.getElementById('bookingForm').addEventListener('submit', function(event) {
        ...
        // Prepare data to send to book_room.php
        ...
        // Send the booking data to book_room.php
        fetch('book_room.php', {
            method: 'POST',
            body: bookingData
        })
        ...
        .then(data => {
            document.getElementById('booking_details').innerHTML = data;
        })
        .catch(error => console.error('Error:', error));
    });

    // Load rooms dynamically from get_room.php
    function loadRooms() {
        fetch('get_room.php')
            .then(response => response.json())
            .then(rooms => {
