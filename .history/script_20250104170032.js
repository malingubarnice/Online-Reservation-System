document.addEventListener('DOMContentLoaded', () => {
    ...
    for (let i = 1; i <= 10; i++) {
        ...
        table.id = `table-${i}`;  // Use backticks for template literals
    ...
    function updateCartDisplay() {
        ...
        cart.forEach(item => {
            ...
            cartItemDiv.innerHTML = `<p>${item.name} - KSh ${item.price.toFixed(2)}</p>`;  // Use backticks for template literals
        ...
        totalDiv.innerHTML = `<p>Total: KSh ${total.toFixed(2)}</p>`;  // Use backticks for template literals
