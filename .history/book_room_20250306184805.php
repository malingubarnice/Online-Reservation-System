...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $contactInfo = $_POST['contact_info'] ?? ''; // Fixed incorrect key
    ...
    $sql = "SELECT 1 FROM bookings WHERE room_id = ? AND NOT (check_out_date <= ? OR check_in_date >= ? )";
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Booking successful! Your Booking ID: $bookingId'); window.location.href='confirmation.php?booking_id=$bookingId';</script>";
    } else {
