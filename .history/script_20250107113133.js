...
function selectRoom(roomId, roomName, price) {
    ...
    document.getElementById('room_id').value = roomId;  // Use hidden input for room ID
    ...
    console.log('room_id:', document.getElementById('room_id').value);
    console.log('room-selection:', document.getElementById('room-selection').value);
...
document.getElementById('room-booking-form').addEventListener('submit', function (event) {
    ...
    console.log('Collected data:', { roomId, checkInDate, checkOutDate, guestCount, contactInfo });

    // Validate form data
    if (!roomId || !checkInDate || !checkOutDate || !guestCount || !contactInfo) {
