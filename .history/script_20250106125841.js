...
// Function to fetch available rooms from get_room.php
function loadRooms() {
    fetch('get_room.php')
      .then(response => response.json())
      .then(data => {
        if (data.status === 'error') {
          document.querySelector('.room-list').innerHTML = `<p>${data.message}</p>`;
        } else {
          let roomListHTML = '';
          data.forEach(room => {
            // Convert price to Kenyan Shillings format
            const priceKES = room.price;  // Assuming room.price is already in KES
            ...
            roomListHTML += `
              <div class="room-card">
                ...
                <p>Price: KSh ${priceKES.toLocaleString()}</p> <!-- Display price in Kenyan Shillings -->
                <button onclick="selectRoom(${room.id}, '${room.room_name}')">Book Now</button>
              </div>
            `;
          });
          document.querySelector('.room-list').innerHTML = roomListHTML;
        }
      })
      .catch(error => console.error('Error fetching rooms:', error));
  }
  
  // Call the loadRooms function when the page loads
  document.addEventListener('DOMContentLoaded', loadRooms);
  
  // Function to handle room selection (for booking)
  function selectRoom(roomId, roomName) {
    // Logic to select the room and proceed with booking
