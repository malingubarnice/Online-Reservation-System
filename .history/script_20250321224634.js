...
function updateTotalCost() {
    ...
    // Loop through all cart items and sum their prices
    $(".cart-items .cart-item").each(function() {
        let itemPrice = $(this).data("price"); // Ensure each item has a 'data-price' attribute
        totalItemsCost += parseFloat(itemPrice) || 0; // Convert to number and sum up
    });

    // Get the delivery cost
    let totalDeliveryCost = $("#delivery-cost").text().replace("Ksh ", "");
    totalDeliveryCost = parseFloat(totalDeliveryCost) || 0; // Default to 0 if invalid

    // Calculate total payable amount
    let totalAmount = totalItemsCost + totalDeliveryCost;

    // Update the total price display in the cart
    $(".total-price").html(`<p>Total: KSh ${totalAmount.toFixed(2)}</p>`);

    // Show the delivery cost in an alert
    alert(`Your Delivery cost is: Ksh ${totalDeliveryCost.toFixed(2)}`);

    return totalAmount; // Return total cost
}
